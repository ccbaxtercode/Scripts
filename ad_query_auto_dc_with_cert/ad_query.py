...
def discover_domain_controller(domain):
    """DNS SRV query ile DC bul
    
    Domain'den otomatik DC hostname'i bulur.
    Sertifika hostname matching için gerekli.
    
    Örnek:
    - Input: test.local.net
    - SRV Query: _ldap._tcp.dc._msdcs.test.local.net
    - Output: dc1.test.local.net
    """
    ...
    try:
        ...
        # Priority/weight sıralamasıyla DC listesi
        dc_list = sorted(answers, key=lambda x: (x.priority, -x.weight))
        
        # İlk DC'yi al
        dc_hostname = str(dc_list[0].target).rstrip('.')
        
        info_log(f"✓ DC bulundu: {dc_hostname}")
        debug_log(f"Toplam DC sayısı: {len(dc_list)}")
        
        for idx, dc in enumerate(dc_list[:3]):  # İlk 3 DC'yi logla
            debug_log(f"  DC #{idx+1}: {str(dc.target).rstrip('.')} (priority={dc.priority}, weight={dc.weight})")
        
        return dc_hostname
        
    except ImportError:
        warn_log("dnspython modülü yok, domain direkt kullanılacak")
        debug_log("pip install dnspython --break-system-packages")
        return domain
    except dns.resolver.NXDOMAIN:
        warn_log(f"DNS SRV kaydı bulunamadı: {srv_record}")
        warn_log("Domain direkt kullanılacak (DC auto-discovery devre dışı)")
        return domain
    except Exception as e:
        warn_log(f"DC discovery başarısız: {e}")
        warn_log("Domain direkt kullanılacak")
        return domain
...
# ============================================
# DC DISCOVERY (Sertifika Hostname Matching için)
# ============================================
...
AD_SERVER = discover_domain_controller(AD_DOMAIN)

if AD_SERVER != AD_DOMAIN:
    info_log(f"Otomatik DC seçildi: {AD_SERVER}")
else:
    info_log(f"Domain direkt kullanılıyor: {AD_DOMAIN}")
...
# SSL Context (AD chain certificate)
...
try:
    ...
    if not os.path.exists(AD_CERT_PATH):
        error_exit(
            f"Sertifika dosyası bulunamadı: {AD_CERT_PATH}",
            "AD_CERT_PATH environment variable'ını kontrol edin"
        )
    
    debug_log(f"Sertifika dosyası: {AD_CERT_PATH}")
    debug_log(f"LDAP Timeout: {LDAP_TIMEOUT} saniye")
    
    # SSL Context oluştur
    ssl_context = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
    ssl_context.verify_mode = ssl.CERT_REQUIRED      # Sertifika doğrulama ZORUNLU
    ssl_context.check_hostname = True                # Hostname kontrolü AÇIK
    ssl_context.load_verify_locations(cafile=AD_CERT_PATH)  # CA chain yükle
    ...
    error_exit(
        f"SSL sertifika yüklenemedi: {AD_CERT_PATH}",
        str(e)
    )

# Server tanımla (DC hostname + timeout)
ldap_server_url = f"ldaps://{AD_SERVER}:636"
debug_log(f"LDAP Server: {ldap_server_url}")
debug_log(f"Connection timeout: {LDAP_TIMEOUT}s")
...
# Bağlantı testi
info_log(f"Bağlanılıyor: {AD_SERVER}")
...
try:
    ...
    if "invalidCredentials" in error_message or "Invalid credentials" in error_message:
        ...
        error_exit("LDAP bağlantı timeout", f"Server {AD_SERVER} erişilebilir değil (timeout: {LDAP_TIMEOUT}s)")
    elif "refused" in error_message.lower():
        error_exit("LDAP bağlantı reddedildi", f"Port 636 kapalı veya {AD_SERVER} erişilebilir değil")
    elif "certificate verify failed" in error_message.lower() or "ssl" in error_message.lower():
        error_exit(
            "SSL sertifika doğrulama başarısız",
            f"Hostname mismatch veya sertifika geçersiz. Server: {AD_SERVER}, Cert: {AD_CERT_PATH}"
        )
    else:
