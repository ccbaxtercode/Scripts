...
function checkGroupAccess(req, res, next) {
  ...
  const userGroups = req.headers['x-remote-groups'] || '';
  const userGroupsList = userGroups.split(',').map(g => g.trim()).filter(g => g);
  
  const hasAccess = userGroupsList.some(group => ALLOWED_GROUPS.includes(group));
  
  if (!hasAccess) {
    console.log(`Access denied for user ${req.headers['x-remote-user']} (groups: ${userGroupsList.join(', ')})`);
    return res.status(403).send(`
      <html>
        <body style="font-family: Arial, sans-serif; text-align: center; padding: 50px;">
          <h1 style="color: #cc0000;">Access Denied</h1>
          <p>You do not have permission to access this page.</p>
          <p>Required group: ${ALLOWED_GROUPS.join(' or ')}</p>
          <p>Your groups: ${userGroupsList.join(', ') || 'None'}</p>
        </body>
      </html>
    `);
  }
