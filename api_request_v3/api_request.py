...
try:
    ...
    print("HATA: 'requests_ntlm' modülü eksik. Kurulum: pip install requests-ntlm")
    sys.exit(1)
...
def make_api_request():
    ...
    # 🔹 Auth credentials (auth type'a göre)
    token = get_env_var("API_TOKEN", required=False)      # Bearer için
    username = get_env_var("API_USERNAME", required=False)  # Basic/NTLM için
    password = get_env_var("API_PASSWORD", required=False)  # Basic/NTLM için
    ...
    # 🔹 Auth tipi kontrol
    if auth_type not in ["basic", "ntlm", "bearer"]:
        logger.error("API_AUTH_TYPE 'basic', 'ntlm' veya 'bearer' olmalı")
        sys.exit(1)

    # 🔹 Auth type'a göre zorunlu alan kontrolü
    if auth_type == "bearer":
        if not token:
            logger.error("Bearer auth için API_TOKEN gerekli")
            sys.exit(1)
    elif auth_type in ["basic", "ntlm"]:
        if not username or not password:
            logger.error(f"{auth_type.upper()} auth için API_USERNAME ve API_PASSWORD gerekli")
            sys.exit(1)
    ...
    if data_json:
        try:
            ...
            # 🔹 Body logging (truncated)
            if logger.isEnabledFor(logging.DEBUG):
                body_str = json.dumps(data, ensure_ascii=False)
                if len(body_str) > 1000:
                    logger.debug(f"Request Body: {body_str[:1000]}... (truncated, total: {len(body_str)} chars)")
                else:
    ...
    # 🔹 GET / DELETE / HEAD için body gönderilmez
    if method in ["GET", "DELETE", "HEAD"] and data is not None:
    ...
    retry_strategy = Retry(
        ...
        allowed_methods=set(valid_methods)
    )
    ...
    if auth_type == "basic":
        ...
        # Bearer token'ı header'a ekle (kullanıcı tanımlamadıysa)
        if "Authorization" not in headers:
            headers["Authorization"] = f"Bearer {token}"
            logger.debug("Auth: Bearer Token (header'a eklendi)")
        else:
            logger.warning("Authorization header zaten tanımlı, API_TOKEN yok sayılıyor")

    logger.info(f"Request: {method} {url} (timeout: {connect_timeout}s connect, {read_timeout}s read, verify_ssl={verify_ssl})")

    # 🔹 API isteği gönder (conditional json parameter)
    try:
        request_kwargs = {
            "method": method,
            "url": url,
        ...
        # Body varsa json parametresini ekle
        if data is not None:
