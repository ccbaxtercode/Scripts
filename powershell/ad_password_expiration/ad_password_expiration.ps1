...
<#
.SYNOPSIS
    Active Directory kullanıcılarının parola expiration sürelerini kontrol eder.
.DESCRIPTION
...
function Get-ADUsersPasswordExpiration {
    <#
    .SYNOPSIS
        Belirtilen domain ve OU'dan kullanıcıların parola expiration bilgilerini okur.
    ...
    Write-Log "Domain: $DomainName - Kullanıcı bilgileri okunuyor..."
    ...
    try {
        ...
        Write-Log "Domain: $DomainName - $($users.Count) aktif kullanıcı bulundu."
        
        foreach ($user in $users) {
            try {
                # Parola expiration date hesapla
                $expiryDate = $null
                $daysRemaining = $null
...
<head>
    <meta charset="UTF-8">
    <title>Parola Expiration Raporu</title>
    <style>
        body {
...
<body>
    <h1>Active Directory Parola Expiration Raporu</h1>
    <div class="info">
        Rapor Tarihi: $(Get-Date -Format "dd.MM.yyyy HH:mm:ss")<br>
        Toplam Kullanıcı Sayısı: $($Data.Count)
    </div>
    <div class="summary">
        <strong>Özet:</strong> Bu rapor, parolası 15 gün veya daha az süre içinde sona erecek kullanıcıları göstermektedir.
    </div>
    <table>
        <thead>
            <tr>
...
try {
    ...
    $usersA = Get-ADUsersPasswordExpiration -DomainName $DomainA -OUPath $OUPathDomainA -DaysThreshold $DaysThreshold
    $allUsers += $usersA
...
# Domain B'den kullanıcıları oku (Encrypted credential ile)
try {
    Write-Log "Domain B credential'ı yükleniyor..."
    ...
    if (Test-Path -Path $EncryptedPasswordFile) {
        ...
        $usersB = Get-ADUsersPasswordExpiration -DomainName $DomainB -OUPath $OUPathDomainB -Credential $credentialB -DaysThreshold $DaysThreshold
        $allUsers += $usersB
        ...
        Write-Log "Dosyayı oluşturmak için: Read-Host -AsSecureString | ConvertFrom-SecureString | Out-File '$EncryptedPasswordFile'" -Level WARNING
    }
}
catch {
